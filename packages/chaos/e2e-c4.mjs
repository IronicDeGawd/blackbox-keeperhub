...
for (const [i, h] of result.txHashes.entries()) console.log(`  attempt ${i}`, explorer(h));
...
for (let i = 1; i <= 8; i++) {
  ...
  for (const x of mine) {
    ...
    if (x.evidence.ruleId === 'R5') console.log('     facts', JSON.stringify(x.evidence.facts));
  }
  if (mine.some((x) => x.class === 'RETRY_STORM')) break;
